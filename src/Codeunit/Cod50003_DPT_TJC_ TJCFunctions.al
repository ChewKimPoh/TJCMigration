codeunit 50003 "TJC Functions"
{
    ...
    procedure CreateTJCSO(SOxml: XMLport TJCOrder): Boolean
    begin
        EXIT(SOxml.IMPORT);
    end;
    ...
    begin
        ...
        IF SalesLine.FINDFIRST THEN
            REPEAT
                IF SalesLine."Return Reason Code" = '' THEN BEGIN
                    ReasonCode := '-';
                END ELSE
    ...
    procedure ImportToNav(var PO: XMLport TJCOrder): Text[30]
    begin
    ...
    begin
        ...
        FOR Li := 1 TO STRLEN(InString) DO BEGIN
            ...
            IF ((LTestChr = '&') OR (LTestChr = Apstrp) OR (LTestChr = '>') OR (LTestChr = '<')) THEN BEGIN
                IF LTestChr = '&' THEN
    ...
    var
        recInvoiceHeader: Record "Sales Header";
        recInvoiceLine: Record "Sales Line";
    ...
    var
        recSalePrice: Record "Sales Price";
        ...
        recCustomer: Record Customer;
        recItem: Record Item;
        recSalesLineDiscount: Record "Sales Line Discount";
    begin
    ...
    var
        recSOHeader: Record "Sales Header";
        recSOLine: Record "Sales Line";
        SalesSetup: Record "Sales & Receivables Setup";
        NoSeriesMgt: Codeunit "NoSeriesManagement";
        SOQuoteNo: Code[10];
        dDocDate: Date;
    ...
    begin
        ...
        IF SalesHeader.GET(SalesHeader."Document Type"::Quote, SONo) THEN BEGIN
            SalesHeader.DELETE(TRUE);
    ...
    begin
        ...
        IF SalesHeader.GET(SalesHeader."Document Type"::Quote, DocNo) THEN BEGIN
            IF SalesHeader.Status = SalesHeader.Status::Open THEN
                EXIT(TRUE)
            ELSE
                EXIT(FALSE);
    ...
    var
        CSalesQuotetoOrder: Codeunit "Sales-Quote to Order";
        recSaleHeader: Record "Sales Header";
        recSaleHeader2: Record "Sales Header";
        retValue: Text[50];
    begin
        retValue := '';
        recSaleHeader.RESET;
    ...
    var
        recSalesQuoteLine: Record "Sales Line";
    begin
    ...
    var
        recReasonCode: Record "Return Reason";
    begin
        RetXML.ADDTEXT('<DPTech>');
        recReasonCode.RESET;
